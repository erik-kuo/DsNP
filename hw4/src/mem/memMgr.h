...
#define toSizeT(t)      (t + (sizeof(size_t) - t % (sizeof(size_t))) % sizeof(size_t))  // TODO
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
#define downtoSizeT(t)  (t - t % sizeof(size_t))  // TODO
...
class MemRecycleList
{
   ...
   T* popFront() {
      // TODO
      T* tmp = _first;
      if(_first)_first = (T*)*(size_t**)_first;
      else _first = 0;
      return tmp;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      if(b != _blockSize && b != 0)
      {
         delete tmp;
         _blockSize = b;
         tmp = new MemBlock<T>(0,_blockSize);
      }
      _activeBlock = tmp;
   }
   // Called by new
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* targetList = &(_recycleList[m]);
      while(targetList->getArrSize() != n)
      {
         if(targetList->getNextList() == 0)break;
         targetList = targetList->getNextList();
      }
      if(targetList->getArrSize() != n)
      {
         MemRecycleList<T>* nextRecycleList = new MemRecycleList<T>(n);
         targetList->setNextList(nextRecycleList);
         targetList = targetList->getNextList();
      }
      return targetList;
   }
   ...
   T* getMem(size_t t) {
      ...
      if(ret)// ret != 0, point to something in recycleList[n]
      {
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
         #endif // MEM_DEBUG
      ...
      //    #endif // MEM_DEBUG
      // TODO
      else if(!_activeBlock->getMem(t, ret)) //not enough
      {
         size_t remainSize = _activeBlock->getRemainSize();
         if(remainSize >= S) //recycle the remain to recyclieList
         {
